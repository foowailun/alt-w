...
ui <- dashboardPage(title = "My Calories", skin = "green", 
        ...
        # Dashboard header                        
        dashboardHeader(title = "My Calories App"),
        ...
        dashboardSidebar(sidebarMenu(id = "sidebar",
          ...
          menuItem(text = "Recommendation", icon = icon("bar-chart-o"), tabName = "recommendation_tab",
                   badgeLabel = "Output", badgeColor = "purple"),
          ...
          tabItems(
            tabItem(tabName = "dashboard",
                    box(radioButtons(inputId = "user_gender", label = "My Gender", choices = c("Male", "Female"), selected = "Male", inline = T),
                        ...
                        actionButton(label = "Reset", inputId = "reset_button")
                        )
            ),
            ...
            tabItem(tabName = "recommendation_tab",
                      ...
                      fluidRow(
                        ...
                        box(verbatimTextOutput("recommended_calories"),title = "Recommended calories per meal (Kcal)", background = "olive",
                            actionButton(label = "Generate Random Meal", inputId = "generate_button")), 

                        box(box(selectInput(choices =  food_calories_table[, food_short_name], inputId = "user_food_intake",
                                            label = NULL, selectize = T, multiple = T,
                                            selected = NULL), background = "red", width = NULL, title = "Recommended For You"),
                            box(tableOutput("diet_breakdown"), background = "olive", width = NULL, title = "Diet Analysis"),
                            box(verbatimTextOutput("balance_calories"), title = "Balance", width = NULL),        
                            title = "Diet Breakdown", collapsible = T, solidHeader = T, background = "olive"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$user_food_intake,  {
    ...
    # coord_polar 
    output$coord_polar <- renderPlot({
      ggplot(food_calories_table[food_short_name %in% input$user_food_intake], aes(fill = food_short_name, y = calorie_in_Kcal, x = "")) + geom_bar(width = 1, stat = "identity")  + coord_polar("y", start=0) + facet_wrap(~products)
    })
